...
point3Df_t *    pNeolixPointCloudData = NULL;    //指向点云数据
...
TOF_ErrorCode_t DisconnectMars04(void) {
	...
	if(NULL != pNeolixPointCloudData){
		free(pNeolixPointCloudData);
		pNeolixPointCloudData =NULL;	
	}
	return rv;
}
...
TOF_ErrorCode_t getMars04AllData(void)
{
	...
	if(NULL == pNeolixPointCloudData){
		LOGI("getMars04AllData: malloc for neolix points cloud data\n");

		pNeolixPointCloudData = (point3Df_t*)malloc(sizeof(point3Df_t)*DEPTHMAP_W*DEPTHMAP_H);
		if(NULL == pNeolixPointCloudData){
			LOGI("getMars04AllData malloc pNeolixPointCloudData failure\n");
			return LTOF_ERROR_NO_MEM;		
		}
		LOGI("[neolix points cloud  data] addr:%p, size:%d\n", pNeolixPointCloudData, sizeof(point3Df_t)*DEPTHMAP_W*DEPTHMAP_H);
	}
	memset(pNeolixPointCloudData,0,sizeof(point3Df_t)*DEPTHMAP_W*DEPTHMAP_H);
	...
	if(rs != LTOF_SUCCESS)
    {
	 ...
	 count = 0;
		 //按x,y,z顺序保存深度数据
			...
			if (pNeolixDepthData != NULL &&  NULL != pNeolixPointCloudData){    //如果jni分配了缓存,我们才给他进行拷贝.
				pNeolixDepthData[i*224+j] = (DepthPixel_t)(frame_data[i*224+j].z*1000f);  //原生的数据是m,最好转换为mm
				pNeolixPointCloudData[count].x     = static_cast<float>(frame_data[count].x*1000f);
				pNeolixPointCloudData[count].y = static_cast<float>(frame_data[count].y*1000f);
				pNeolixPointCloudData[count].z = static_cast<float>(frame_data[count].z*1000f);
			}
#ifdef NATIVE_WRAPPER_DEBUG
	//打印中心点
			if (i == 86 && j == 112){
		...
		mars04_all_data.PointCloudData = pNeolixPointCloudData;
