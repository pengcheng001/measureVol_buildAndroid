...
static neolix::pointcloudData  pointsCloudData;
//存储算法建模时候的参数.
...
static neolix::point3Df	*pCloudData = NULL;     //指向点云数据缓存
...
void getMars04DepthDataCb(Mars04AllData_t& mars04AllData)
{
	static neolix::depthData DataTmp;
        static neolix::point3Df* pointCloudTmp;
	...
	case VIEW_DEPTH_MODE:
		...
		if (NULL != mars04AllData.pDepthData){
			DataTmp.data = mars04AllData.pDepthData;
                        pointCloudTmp = (neolix::point3Df*)(mars04AllData.PointCloudData);
		}
		else {
	...
	if (cur_view_mode == VIEW_DEPTH_MODE) {
			if (!isCubing) {
				...
				memcpy(pCloudData, pointCloudTmp,mars04AllData.depth_data_size/sizeof(DepthPixel_t)*sizeof(neolix::point3Df));
  ...
  (JNIEnv *env, jclass jc, jobject dev_obj)
{
	...
	///开启点云缓存区域
        pCloudData = new neolix::point3Df[devInfo.DepthFrameHeight*devInfo.DepthFrameWidth];
	if(NULL == pCloudData){
	LOGE("new points Cloud mem fail\n");
        return false;
    }
	//分配存储伪彩色数据的区域.
	...
	pointsCloudData.width = devInfo.DepthFrameWidth;
	pointsCloudData.height = devInfo.DepthFrameHeight;
	pointsCloudData.data = pCloudData;
...
(JNIEnv *je, jclass jc)
{
	...
	//释放资源.
	if (pColorImage != NULL)delete[] pColorImage;
	if (pRGBAData != NULL) delete[] pRGBAData;
	if (pDepthData != NULL)	delete[] pDepthData;
	if (NULL != pCloudData) delete[] pCloudData;
	pColorImage = NULL;
	pRGBAData = NULL;
	pDepthData = NULL;
	pCloudData = NULL;
 ...
 */
jboolean JNICALL EXTAND_FUNC_NAME(CLASS_NAME, getCurrentVolumeMobile)
  (JNIEnv *env, jclass jc, jobject volumeobj, jint method)
{
    jclass clazz;
    jfieldID fid;
    neolix::vol t;
	...
	ret = neolix::measureVol3(pointsCloudData, t, method);
	isCubing = false;
	 if (ret)
 ...
 * Signature: ()Lcom/neolix/neolixcubing/Volume;
 */
jboolean JNICALL EXTAND_FUNC_NAME(CLASS_NAME, getCurrentVolume2)
  (JNIEnv *env, jclass jc, jobject volumeobj, jint method)
{
    jclass clazz;
    jfieldID fid;
